...
DBounds2D<GetBiggerType<TLeft, TRight>> 
operator+(const DBounds2D<TLeft>& lhs, const DBounds2D<TRight>& rhs) noexcept 
{
  using TBiggerType = GetBiggerType<TLeft, TRight>;
  return DBounds2D<TBiggerType>{
...
bool operator==(const DBounds2D<TType>& lhs, const DBounds2D<TType>& rhs) noexcept
{
  return 
